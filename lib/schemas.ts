...
export const tradeSchema = z.object({
  ...
  asset_klasse: z.enum(['Index', 'Rohstoff', 'Krypto', 'Aktie', 'FX'], { error: 'Pflichtfeld' }),
  richtung: z.enum(['LONG', 'SHORT'], { error: 'Pflichtfeld' }),
  einstiegspreis: z.number({ error: 'Pflichtfeld' }).positive('Muss positiv sein'),
  stop_loss: z.number({ error: 'Ungültige Zahl' }).nullable().optional(),
  tp1: z.number({ error: 'Ungültige Zahl' }).nullable().optional(),
  tp2: z.number({ error: 'Ungültige Zahl' }).nullable().optional(),
  tp3: z.number({ error: 'Ungültige Zahl' }).nullable().optional(),
  tp4: z.number({ error: 'Ungültige Zahl' }).nullable().optional(),
  status: z.enum([
    'Entwurf',
    'Aktiv',
    'Geschlossen',
  ...
  gewichtung: z.number({ error: 'Pflichtfeld' }).min(0, 'Min. 0%').max(1, 'Max. 100%'),
