...
// Aggregates all landing_events into per-day summaries.
// Cached for 5 minutes via Next.js ISR cache — no re-fetching of thousands
// of raw events on every dashboard load.
...
type UtmEntry = {
  ...
  campaign_id: string; views: number; clicks: number;
}
...
export async function getAnalytics() {
  ...
  if (!data) return { error: 'Keine Daten verfügbar' }
